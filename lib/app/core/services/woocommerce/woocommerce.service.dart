...
class WooCommerceService {
  ...
  Future<bool> getInStockQueryParam() async {
    dynamic configApp = await SessionService.getItem(key: 'app_config');
    configApp = json.decode(configApp);

    return Future.value(configApp['show_soldout_products'] == 1 ? false : true);
  }
  ...
  Future<dynamic> getRecommendedProducts({
    ...
    bool inStockParam = await getInStockQueryParam();
    ...
    try {
      if (ConfigService.USE_OWN_WOOCOMMERCE_API) {
        ...
        if (inStockParam) params.addAll({'in_stock': 'true'});
        params.addAll({
          'consumer_key': '${ConfigService.WC_CONSUMER_KEY}',
          'consumer_secret': '${ConfigService.WC_CONSUMER_SECRET}'
        });
        if (categoryId != null)
          params.addAll({'category': '${categoryId.toString()}'});
        products = await HttpService.get(
            url: endpoint, http: true, queryParams: params);
      }
  ...
  Future<dynamic> getCategories() async {
    ...
    try {
      ...
      if (ConfigService.USE_OWN_WOOCOMMERCE_API) {
        ...
        wcCategories = await HttpService.get(
            url: endpoint, http: true, queryParams: params);
      }
  ...
  Future saveOrder({Map couponData, double couponDiscountTotal}) async {
    ...
    if (userData['payment_method'] == 'upon-delivery') {
      if (userData['subpayment_method'] == 'dataphone') {
        paymentMethod = 'alg_custom_gateway_1';
        paymentMethodTitle = 'Pago a contra entrega con datáfono';
        orderStatus = 'on-hold';
      } else if (userData['subpayment_method'] == 'cash') {
        paymentMethod = 'cod';
      ...
      paymentMethodTitle =
          'Checkout ePayco (Tarjetas de crédito,debito,efectivo)';
      orderStatus = 'pending';
    }
